...
| 后端 | 吞吐 | 1099 条切片 |
|---|---|---|
| 本地 CPU（`device="cpu"`，1024 维） | **0.57 texts/s** | **~32 分钟** |
| **出货配置**（batch 32 / 3072 维 / base64） | **8.4 texts/s** | **~2.2 分钟** |
| 参考：batch 512 / 1024 维 / JSON | 21.5 texts/s | ~52 秒 |

出货配置相对本地 CPU 约 **15 倍**改善。

8.4 texts/s 是**唯一的端到端数字**：`dataset/swe-chat-reasoning-sample-50.jsonl`
完整 ingest，1099 条切片，向量化 130.8 秒（总 wall 132.3 秒），含全部重试与 429
退避开销。早前记录的 5.7 texts/s 为单批次采样，未摊薄连接建立成本，已作废。

注意它**不是** 38 倍——那是 batch 512 / 1024 维下的数字。出货配置为迁就网关做了两处让步：
批次 512→32（请求数 ×16）、维度 1024→3072（传输量 ×3）。这是自觉的取舍：
被网关拦截等于 0 texts/s，15 倍已足够跨过可用性门槛。

> 关于 MPS：本开发机 MPS 可用，MPS 下本地可达 16–20 texts/s，高于出货配置。
...
- **网关阈值为用户口述的约 1 MB**：未实地压测确认精确值，也未确认该限制作用于响应体、请求体还是两者。batch 32 实测响应体 0.50 MB（额度 50%），留有 2× 余量，可容纳一定误差。
- **代理负载敏感**：所有吞吐数字测于特定时段，代理负载变化时会浮动。8.4 texts/s 为单次完整 ingest 观测，未做多轮取中位。
- **429 退避参数未经调优**：2/4/8/16/32s 是按「配额窗口通常 60 秒级」推断的，未实测 Azure S0 层的实际窗口长度。实测该配置能让 1099 条切片跑完，但未验证它是最优或在更大语料下仍足够。
- **`mount_threshold` 已知失准且未修**：见 §4.8。真假分布重叠，0.50 漏 11 对真关系中的 9 对。本次不改是自觉决定，但这意味着 `uxflow-evolve` 的标签自演化在本后端下会大量误判。

### 已消解的原未验证项

- ~~本地 Qwen 基准未测得~~ → 已测：CPU 0.57 texts/s（§1）
- ~~21.5 texts/s 需按出货配置复测~~ → 已测：完整 ingest 8.4 texts/s（§1）。中途记录过的 5.7 texts/s 为单批次采样，已作废

## 8. 验收标准与结果

| # | 标准 | 结果 |
|---|---|---|
| 1 | `UXFLOW_EMBED_BACKEND=api` 且仅配置 `LITELLM_BASE`/`LITELLM_KEY` 时可正常启动 | ✅ 实测，`.env` 零改动 |
| 2 | `ApiEmbedder` 返回 3072 维 L2 归一化向量 | ✅ 落库 1099 行全为 3072 维，范数 1.0000 |
| 3 | base64 与 float 两种响应形态均能正确解析 | ✅ 单测覆盖，含解码失败路径 |
| 4 | 超时/5xx/429 触发重试；4xx 立即抛出 | ✅ 单测覆盖，含延迟值断言 |
| 5 | `UXFLOW_EMBED_BATCH` / `_TIMEOUT` / `_MAX_TRIES` 生效 | ✅ 单测覆盖；另增 `_RATELIMIT_BASE` / `_CAP` |
| 6 | `local` / `fake` 后端行为与改动前一致 | ✅ `LocalEmbedder` 报 32，与原 `_EMBED_CHUNK` 等价 |
| 7 | 全量测试通过 | ✅ 636 passed, 1 skipped；ruff clean |
| 8 | 真实轨迹端到端 ingest，记录吞吐与响应体大小 | ✅ 1099 条 / 132.3s / 8.4 texts/s；响应体 0.50 MB（网关额度 50%） |

真实 ingest 补充核验：

- 维度分布单一（1099 行全 3072 维），无混维
- 零向量恰好 19 条，与 §4.9 统计的空文本切片数一致
- 召回返回语义相关结果（查询 "fix a syntax error in a python file" 命中的切片
  确为语法错误排查），非随机噪声

### 真实数据发现的两个缺陷（单元测试未能覆盖）

两者都是 636 个单元测试全绿、但真实 ingest 一跑即崩的问题，记录以说明
「用真实数据跑一次」不可省略：

1. **Azure S0 配额 429**（§4.5）—— 需持续负载才触发（第 12 个请求，t+29s），
   mock 无法产生。原设计对 429 沿用固定 1s 重试，5 秒内耗尽全部尝试。
2. **空文本切片 400**（§4.9）—— 需真实语料才产生空串（1099 条中 19 条），
   合成测试文本永远非空。API 拒绝空输入且 400 不重试，一条即可杀死整个 ingest。
